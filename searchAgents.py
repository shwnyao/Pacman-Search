        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                if (nextx, nexty) in self.corners and (nextx, nexty) not in visitedCorners:
                    updatedVisitedCorners = visitedCorners + ((nextx, nexty),)
                    succ = ((nextx, nexty), updatedVisitedCorners)
                    successors.append((succ, action, 1))
                else:
                    succ = ((nextx, nexty), visitedCorners)
                    successors.append((succ, action, 1))
...
def cornersHeuristic(state, problem):
    ...
    heuristic = 0
    while unvisited:
        dist, corner = min((util.manhattanDistance(pos, corner), corner)
...
def foodHeuristic(state, problem):
    ...
    heuristic = max(mazeDistance(position, food, problem.startingGameState)
                    for food in foodList)

    # while foodList:
    #     dist, food = min((util.manhattanDistance(position, food), food)
    #                      for food in foodList)
    #     heuristic += dist
    #     pos = food
